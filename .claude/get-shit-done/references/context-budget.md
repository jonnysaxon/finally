...
The keys live in `.claude/settings.json` (project) or `/Users/jonnysaxon/Library/Mobile Documents/com~apple~CloudDocs/jonnys/my stuff/mac pro laptop/AI/AI Coder Complete Claude Code and Coding Agents Course/projects/finally/.claude/settings.json` (global) — **not** in `.planning/config.json`:
